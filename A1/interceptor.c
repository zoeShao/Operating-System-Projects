...
MODULE_AUTHOR("Qiyuan He, Yuting Shao");
MODULE_LICENSE("GPL");
...
asmlinkage long my_exit_group(struct pt_regs reg){
	...
	// Remove current's pid from all lists.
	del_pid(current->pid);
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	if (table[reg.ax].monitored == 2) {
		// All pids are monitored, so just log the message.
		log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
		
	} else if (table[reg.ax].monitored == 1) {
		// When some pids are monitored, check whether the current pid has been monitored or not.
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	//-------------------Check for valid cmd value-------------------------
	// 0: the cmd must be one of the four cases.
	if ((cmd != REQUEST_SYSCALL_INTERCEPT) && (cmd != REQUEST_SYSCALL_RELEASE) && (cmd!= REQUEST_START_MONITORING) && (cmd != REQUEST_STOP_MONITORING)){
		return -EINVAL;
	}

	//------------------A.Check for valid syscall value---------------------
	// A(a) The syscall must be valid.
	//(i.e. not negative, not > NR_syscalls-1, and not MY_CUSTOM_SYSCALL itself)
	if ((syscall < 0) || (syscall > NR_syscalls - 1) || (syscall == MY_CUSTOM_SYSCALL)){
		return -EINVAL;
	}

	// A(b) The last two cases Pid should be valid. 
	if ((cmd == REQUEST_START_MONITORING) || (cmd == REQUEST_STOP_MONITORING)){
		// It cannot be a negative integer.
		if (pid < 0){
			return -EINVAL;
		}
		//it must be an existing pid. (i.e. Belongs to a valid process)
		if (pid > 0){
			if (pid_task(find_vpid(pid), PIDTYPE_PID) == NULL){// when doesn't belong to a valid process
				return -EINVAL;
			}
		}
	}

	//------------------B.Check for Right permission-------------------------
	// B(a) Right permission for first two cases: must be root.
	...
	// B(b) Right permission for last two cases:
	if ((cmd==REQUEST_START_MONITORING) || (cmd == REQUEST_STOP_MONITORING)){
		if (current_uid() != 0){	// calling process not root
			// - if 'pid' is 0 and the calling process is not root, then access is denied
			if (pid==0) {	// try to monitor/demonitor all when not root
				return -EPERM;
			}
			// - also, if not root, then check if the 'pid' requested is owned by the calling process		
	...
	//------------------C.Check for correct context-------------------------
	spin_lock(&my_table_lock);	

	// C(a) - cannot de-intercept a syscall that hasn't been intercepted
	...
	// C(b) - cannot stop monitor a pid that hasn't been monitored/syscall not intercepted.
	if (cmd == REQUEST_STOP_MONITORING){
		if (table[syscall].intercepted == 0){	// stop monitor unintercepted.
			spin_unlock(&my_table_lock);	
			return -EINVAL;
		}
		if (table[syscall].monitored == 0){	// stop monitor unmonitored syscall
			spin_unlock(&my_table_lock);	
			return -EINVAL;
		}
		if ((table[syscall].monitored == 1) && (check_pid_monitored(syscall, pid) == 0)){	// monitored syscall, pid not monitored
			spin_unlock(&my_table_lock);	
			return -EINVAL;
		}
		// In this cases we only allow: moniotored = 1 and pid in syscall moniotred list; moniotred = 2.
	}

	// C - extra - cannot request monitor when not intercepted
	// If the system call has not been intercepted yet, a command to start monitoring a pid for that syscall is also invalid. 
	...
	//---------------------D.Check for EBUSY-----------------------------
	// D(a) intercepting a syscall already intercepted.
	...
	// D(b) monitoring a pid that is already being monitored.
	if (cmd == REQUEST_START_MONITORING){
	...
	//=========start to implement syscall with four types of cmds=========
	// First case: REQUEST_SYSCALL_INTERCEPT:
	if (cmd == REQUEST_SYSCALL_INTERCEPT) {
		...
		// Replaced the corresponding entry in sys_call_table with interceptor func.
		sys_call_table[syscall] = interceptor;
		set_addr_ro((unsigned long)sys_call_table);
		spin_unlock(&sys_call_table_lock);

	// Second case: REQUEST_SYSCALL_RELEASE:
		...
		spin_lock(&sys_call_table_lock);
		set_addr_rw((unsigned long)sys_call_table);
		// Restored the original saved system call in the system call 
		//table in its corresponding position.
		sys_call_table[syscall] = table[syscall].f;
		set_addr_ro((unsigned long)sys_call_table);
		spin_unlock(&sys_call_table_lock);

	// Third case: REQUEST_START_MONITORING:
	} else if (cmd == REQUEST_START_MONITORING) {
		if (pid == 0) {
			spin_lock(&my_table_lock);
			// Request monitoring all pids, need to first destroy then set the monitored value to 2.
			destroy_list(syscall);
			...
			if (table[syscall].monitored == 2) {
				...
				// E.A pid cannot be added to a monitored list when no memory being available.
	...
	// Fourth case: REQUEST_STOP_MONITORING:
		...
		if (pid == 0) {
			...
			spin_lock(&my_table_lock);
			if (table[syscall].monitored == 2 || del_pid_sysc(pid, syscall) != 0) {
				spin_unlock(&my_table_lock);
				return -EINVAL;
			}
			spin_unlock(&my_table_lock);
...
static int init_function(void) {
	...
	// Replace the system call table entry for the MY_CUSTOM_SYSCALL number, with the 
	//my_syscall function.
	sys_call_table[MY_CUSTOM_SYSCALL] = my_syscall; 
	// Save the original exit_group function.
	orig_exit_group = sys_call_table[__NR_exit_group];
	// hijack the exit_group system call(with number __NR_exit_group), by replacing it 
	//in the system call table with your own custom function my_exit_group.
...
static void exit_function(void){
	...
	spin_lock(&sys_call_table_lock);
	set_addr_rw((unsigned long)sys_call_table);
	// Restore this system call to its original routine, when the module is released.
	sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall;
	// Restore it when kernel module is unloaded.
